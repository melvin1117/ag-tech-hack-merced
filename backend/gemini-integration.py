...
def generate_gemini_insights(sensor_data, current_crop, lat=None, lon=None, image_path=None, model_name="gemini-1.5-pro"):
    ...
    if not GEMINI_API_KEY:
        ...
        return {
            ...
            'soil_health': {
                ...
                'P': {'status': 'unknown'},
                'K': {'status': 'unknown'},
                'derived_metrics': {'growth_potential': {'status': 'Unknown - API key missing'}}
            },
...
def main():
    ...
    farm_data = generate_gemini_insights(sensor_data, current_crop, lat, lon, image_path)
